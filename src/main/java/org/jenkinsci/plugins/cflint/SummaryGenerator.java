...
public final class SummaryGenerator {
    ...
    public static String createSummary(final LintResult result) {
        ...
        summary.append("Lint: ");
        if (issues > 0) {
            summary.append("<a href=\"CFLintResult\">");
        }
        summary.append(Messages.CFLint_ResultAction_Issues(issues));
        if (issues > 0) {
            summary.append("</a>");
        }
        summary.append(".");
    ...
    public static String createDeltaMessage(final LintResult result) {
        ...
        if (result.getNumberOfNewWarnings() > 0) {
            summary.append("<li><a href=\"CFLintResult/new\">");
            summary.append(Messages.CFLint_ResultAction_NewIssues(result.getNumberOfNewWarnings()));
            summary.append("</a></li>");
        }
        if (result.getNumberOfFixedWarnings() > 0) {
            summary.append("<li><a href=\"CFLintResult/fixed\">");
            summary.append(Messages.CFLint_ResultAction_FixedIssues(result.getNumberOfFixedWarnings()));
            summary.append("</a></li>");
        }
